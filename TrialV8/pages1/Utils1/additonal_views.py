...
import pandas as pd
from pages1.Utils1.graphUtils import nx_Digraph, show_yfiles_graph, nx_to_yfiles_graph
from pages1.Utils1.config import PPR_ATTRS, ENG_ATTRS, SUST_ATTRS, edgeType_by_view, global_nodes_list, \
    VIEW_ATTRS_MAPPING
from pages1.Utils1.data_transform import nodes_by_class, edges_by_type, extract_numeric, are_attributes_same, \
    remove_links_of_node
from pages1.Utils1.req_check import get_matching_elements_by_attribute
...
def show(ppr_nodes: dict, nodes: dict, links: list, selected_view: str, locked: bool = False):
    ...
    with col1:
        ...
        if active_panel == "🏗️ Edit Network":
            view_radio = st.radio("Select the operation", [f"Add or Modify Nodes",
                                                           f"Create Edges"],
                        ...
                        if selected_node:
                            if selected_node in nodes:
                                for attr_item in nodes[selected_node].get("attributes", []):
                                    existing_attrs[attr_item["attr_name"]] = extract_numeric(
                                        attr_item.get("value", "0"))
                        if VIEW_ATTRS[
                            node_type]:  # Check if there are attributes defined for this element type
                            available_attrs = list(VIEW_ATTRS[node_type].keys())
                            for attr in available_attrs:
                                attrs_info = VIEW_ATTRS[node_type][attr]
                                unit = attrs_info["unit"]
                                st.text("Adjust Attribute Value(s):")
                                ...
                                with col1_1:
                                    value = st.number_input(f"{attr.replace('_', ' ').title()}",
                                                            value=default_val,
                                                            disabled=locked,
                                                            min_value=0.0, step=0.1, format="%f",
                                                            key=f"{selected_node}_{attr}_value")
                                with col1_2:
                                    st.text_input("Unit", value=unit, disabled=True, key=f"{attr}_unit")
                                selected_ppr_attrs.append({"attr_name": attr, "value": f"{value} {unit}"})

                        elem_data = {}
                        submitted = st.form_submit_button("Add/Update Node", disabled=locked,
                                                          key="add_node_button_view")
                        ...
                        if submitted:
                            if selected_node is None:
                                ...
                                if are_attributes_same(old_attrs_numeric, selected_ppr_attrs):
                                    st.error(
                                        f"No changes detected for node **{selected_node}**. Attributes are the same as before.")
                                else:
                    ...
                    with st.form(key=f"create_{view_name}_edge_form"):
                        ...
                        if submitted_edge:
                            if source_node is None or target_node is None:
                                st.error(
                                    "🚨 **Validation Error:** You must select both a Source and a Target node to create a link.")
                                ...
                                duplicate_exists = any(link for link in links if
                                                       link["source"] == source_node and link["target"] == target_node)
                                if duplicate_exists:
                                    st.error(
                                    ...
                                    st.success(
                                        f"Edge of type **{view_edge_type}** created between {source_node} and {target_node}!")
                                    st.rerun()

        elif active_panel == "🗑️ Delete Components":
            view_del_radio = st.radio("Select the operation",
                                      [f"Delete Node from {view_name} view", f"Delete Edge from {view_name} view"],
                                      key=f"{view_name}_del_node_or_edge")
            with st.container(border=True, key="view_delete_container"):
                if view_del_radio == f"Delete Node from {view_name} view":
                    ...
                    view_del_node_type = st.selectbox("Node Type", global_nodes_list,
                                                      key=f"del_{view_name}_node_type_select")
                    with st.form(key=f"delete_{view_name}_node_form"):
                        view_node_to_del = st.selectbox("Select Node to Delete",
                                                        options=view_grouped_nodes.get(view_del_node_type, []),
                                                        index=None,
                        ...
                        if submitted_del_view_node:
                            if view_node_to_del is None:
                                ...
                                remove_links_of_node(links,
                                                     view_node_to_del)  # Remove all links associated with the node
                                nodes.pop(view_node_to_del,
                                          None)  # Remove the node from the nodes dictionary  # Iterate backwards to avoid index issues when deleting
                                st.success(
                                    f"Node **{view_node_to_del}** of type **{view_del_node_type}** deleted from {view_name} view!")
                                st.rerun()

                if view_del_radio == f"Delete Edge from {view_name} view":
                    st.markdown(f"#### Delete Edge from {view_name} view")
                    view_del_edge_type = st.selectbox("Edge Type", VIEW_EDGE_TYPES,
                                                      key=f"del_{view_name}_edge_type_select")
                    with st.form(key=f"delete_{view_name}_edge_form"):
                        view_edge_to_del = st.selectbox("Select Edge to Delete",
                                                        options=view_grouped_edges.get(view_del_edge_type, []),
                                                        index=None,
                        ...
                        if submitted_del_view_edge:
                            if view_edge_to_del is None:
                                ...
                                for i in range(len(links) - 1, -1,
                                               -1):  # Iterate backwards to avoid index issues when deleting
                                    if links[i]["name"] == view_edge_to_del and links[i]["type"] == view_del_edge_type:
                                        del links[i]  # Remove the link from the links list
                                        st.success(
                                            f"Edge **{view_edge_to_del}** of type **{view_del_edge_type}** deleted from {view_name} view!")
                                        st.rerun()
                                        break
        # with req_check:
        elif active_panel == "✅ Requirement Check":
            st.write("Requirement Check")
            node_type = st.selectbox("Node Type", global_nodes_list, key=f"{view_name}_node_type_select_req_check")
            selected_attr = st.selectbox(f"Select attribute to be checked for class: **{node_type}**",
                                         options=VIEW_ATTRS[node_type], key=f"{view_name}_attr_select_req_check")
            req_check_ops = ["equal to (=)", "less than (<)", "greater than (>)", "less than or equal to (<=)",
                             "greater than or equal to (>=)"]
            fun = st.selectbox("Value should be...", options=req_check_ops)
            val = st.number_input("Enter the value", disabled=locked, min_value=0.0, step=0.1, format="%f",
                                  key=f"{view_name}_req_check_value")

            if st.button(f"Show elements with '{selected_attr}' {fun} ***{val}***"):
                result_df = get_matching_elements_by_attribute(
                    nodes=nodes,
                    node_type=node_type,
                ...
                if not result_df.empty:
                    st.success(f"Found {len(result_df)} matching element(s).")
                    st.dataframe(result_df, use_container_width=True)
                else:
                    st.warning("No matching elements found.")
    with col2:
        with st.container(border=True):
            st.info(f"{view_name} View Graph Visualization")
