...
from pages1.Utils1.req_check import sim_struc_elem, isolated_check, check_axle_wheel_rule
## PPR Viewpoint visualisation, CRUD operations and requirement check initialisation
...
def show(nodes: dict, links: list, engineering_nodes: dict, engineering_edges: list,
    ...
    with c1:
        ...
        if active_panel == "🏗️ Edit Network":
            ppr_radio = st.radio("Select the operation", ["Create PPR Node", "Edit PPR Node", "Create PPR Edge", "Edit Cardinality"],
                                 key="create_node_or_edge_or_check")
            with st.container(border=True, key="ppr_node_container"):
                    ...
                    with st.form(key="create_ppr_node_form"):
                        ...
                        if PPR_ATTRS[node_type]:  # Check if there are attributes defined for this element type
                            ...
                            for attr in available_attrs:
                                ...
                                chosen_value = st.selectbox(f"Choose {attr} attribute", options=attr_options,
                                                            index=None, key=f"{node_type}_{attr}")
                        ...
                        if submitted:
                            ...
                            if not node_name.strip():
                                ...
                                st.error(
                                    "🚨 **Validation Error:** A node with this name already exists. Please choose a different name.")
                            else:
                                elem_data = {
                                    "name": node_name,
                                    "class": node_type,
                ...
                if ppr_radio == "Edit PPR Node":
                    ...
                    node_name = st.selectbox("Choose Node to edit", options=grouped_nodes.get(node_type, []),
                                             disabled=locked, key="ppr_node_name_select")
                    ...
                    with st.form(key="edit_ppr_node_form"):
                        ...
                        new_parent_node = st.selectbox("Change Parent of the node", options=parent_candidates,
                                                       disabled=locked, index=None)
                        selected_ppr_attrs = []
                ...
                if ppr_radio == "Edit Cardinality":
                    st.markdown("#### Edit Cardinality")

                    with st.form(key="edit_ppr_edge_form"):
                        edge_type = st.selectbox(
                            "Edge Type",
                            edgeType_by_view[selected_view],
                            key="edit_edge_type_select"
                        )
                        ...
                        new_cardinality = st.number_input(
                            "New Cardinality",
                            min_value=1,
                            step=1,
                            value=None,
                            key="edit_edge_cardinality"
                        )

                        submitted = st.form_submit_button("Update Edge", disabled=locked)
                        ...
                        if submitted:
                            ...
                            # ----------------------------
                            if source_node is None or target_node is None:
                                st.error("🚨 You must select both Source and Target nodes.")
                                ...
                                for link in links:
                                    if (
                                            link["source"] == source_node and
                                            link["target"] == target_node and
                                            link["type"] == edge_type
                                    ):
                                        matching_edge = link
                                        break
                                ...
                                if not matching_edge:
                                    st.error("🚨 No such edge exists to edit.")
                                else:
                                    # update cardinality
                                    matching_edge["cardinality"] = new_cardinality

                                    st.success(
                                        f"Edge {source_node} → {target_node} updated with cardinality {new_cardinality}!"
                                    )
                                    st.rerun()
            ...
            with st.container(border=True, key="ppr_del_container"):
                if ppr_del_radio == "Delete Node":
                    ...
                    with st.form(key="delete_ppr_node_form"):
                        node_to_delete = st.selectbox("Select Node to Delete",
                                                      options=grouped_nodes.get(del_node_type, []), index=None,
                                                      key="node_to_del_select")  # Replace with actual list of nodes from the graph
                        ...
                        if submitted_del_node:
                            if node_to_delete is None:
                                ...
                                remove_links_of_node(engineering_edges,
                                                     node_to_delete)  # Also remove from engineering edges
                ...
                if ppr_del_radio == "Delete Edge":
                    ...
                    del_edge_type = st.selectbox("Edge Type", edgeType_by_view[selected_view],
                                                 key="del_edge_type_select")
                    with st.form(key="delete_ppr_edge_form"):
                        edge_to_delete = st.selectbox("Select Edge to Delete", options=link_names, index=None,
                                                      key="edge_to_del_select")  # Replace with actual list of edges from the graph
        ...
        # with tab_req_check:
            ...
            check_axle_wheel_rule(nx_ppr_graph)
