...
from pages1.Utils1.graphUtils import nx_Digraph, node_cardinality_mapping, show_yfiles_graph, nx_to_yfiles_graph, ppr_req_check
...
def show(nodes: dict, links: list, engineering_nodes: dict, engineering_edges: list,
        ...
        with c1:
            # tab_edit, tab_delete, tab_req_check = st.tabs(["🏗️ Edit Network", "🗑️ Delete Components", "Requirement Check"])
            ...
            if active_panel == "🏗️ Edit Network":
                ppr_radio=st.radio("What do you want to do?",["Create PPR Node", "Edit PPR Node", "Create PPR Edge"], key="create_node_or_edge_or_check")
                with st.container(border=True,key="ppr_node_container"):
                                ...
                                if source_node is None or target_node is None:
                                    ...
                                    if duplicate_exists:
                                        ...
                                        links.append({"name":source_node + "_to_" + target_node,"source": source_node,
                                                    "target": target_node,"type": edge_type})
                    ...
                    if ppr_radio == "Edit PPR Node":
                        ...
                        with st.form(key="edit_ppr_node_form"):
                            # if node_name:
                            #     selected_node = nodes.get(node_name, {})
                            #     st.write("### Existing Properties of Selected Node")
                            #     # Show only the useful basic properties
                            #     basic_info = {
                            #         "Property": ["Name","Class", "Parent"],
                            #         "Value": [
                            #             selected_node.get("name", ""),
                            #             selected_node.get("class", ""),
                            #             selected_node.get("parent", "")
                            #         ]
                            #     }
                            #
                            ...
                            #     else:
                            #         st.info("No attributes available for this node.")
                            ...
                            if PPR_ATTRS[new_class]:  # Check if there are attributes defined for this element type
                                ...
                                for attr in available_attrs:
                                    ...
                                    selected_ppr_attrs.append({"attr_name": attr, "value": chosen_value})
                            # st.write(nodes)
                            # st.write(str(node_name))
                            # st.write(str(new_class))
                            # st.write(str(new_node_name))
                            # st.write(nodes)
                            # st.write(links)
                            elem_data = {}
                            ...
                            #Also check if there are duplicate node names here before editing the element. Check if empty node name is allowed or not.
                                ...
                                # elif new_node_name in all_nodes:
                                #     st.error("🚨 **Validation Error:** A node with this name already exists. Please choose a different name.")
                                else:
                                    elem_data = {
                                        "name": new_node_name,
