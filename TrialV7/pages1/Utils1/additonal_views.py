...
def show(ppr_nodes: dict, nodes: dict,links: list, selected_view: str, locked: bool = False):      
        ...
        with col1:
            ...
            if active_panel == "🏗️ Edit Network":
                ...
                view_del_radio=st.radio("Select the operation",[f"Delete Node from {view_name} view", f"Delete Edge from {view_name} view"], key=f"{view_name}_del_node_or_edge")
            ...
            # with req_check:
                ...
                selected_attr = st.selectbox(f"Select attribute to be checked for class: **{node_type}**", options=VIEW_ATTRS[node_type], key=f"{view_name}_attr_select_req_check")
                req_check_ops = ["equal to (=)", "less than (<)", "greater than (>)", "less than or equal to (<=)", "greater than or equal to (>=)"]
                fun = st.selectbox("Value should be...", options=req_check_ops)
                val = st.number_input("Enter the value",disabled=locked, min_value=0.0,step=0.1,format="%f",key=f"{view_name}_req_check_value")
